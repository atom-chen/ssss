...
function FightLayer:ctor(mapInfo, size, layer)
...
function FightLayer:createBuildings(builds, size)
	...
	self.situationLayer:setFlagNum(c2, c3)
...
function FightLayer:createGenerals(list)
	...
	for _, v in pairs(gls) do
		...
		if generalId ~= 0 then
			...
			general.FSM:bindStateCallback(kRoleStateClear, function() 
				...
				self.situationLayer:generalDead(general.owner)
	...
	self.situationLayer:setGeneralNum(c1, c2)
end
...
function FightLayer:updateBuildLayout(from, to)
	...
	self.situationLayer:setFlagNum(self.buildPlayer, self.buildEnemy)
